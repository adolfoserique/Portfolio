...
float temp_sum = 0;                                                     // Sum of temperature (Celsius) for mean calculation
float temp_display = 0;                                                 // Temperature (Celsius) for display provided by sensor
...
int temp_count = 0;                                                     // Count to extract the temperature mean
...
// Wi-Fi Connection
const char *ssid = "ADOLFO";
const char *password = "992651818a";
  ...
  void onWrite(BLECharacteristic *pCharacteristic){
    ...
    if (rxValue.length() > 0) {
      ...
      if(rxValue.indexOf("S") != -1){
        ...
        if(select_mode == 0 && rep == HIGH && temp_select_cal < temp_max){
       ...
       if(select_mode == 0 && rep == HIGH && temp_select_cal > temp_min){
...
void temp_select_mode(void){
  ...
  if(temp_select_cal > 70 && temp_select_cal <= 80){
  ...
  }else if(temp_select_cal > 90 && temp_select_cal <= 95){
    ...
    delay(2000 - refresh_rate);

    temp_display = thermocouple.readCelsius();
    temp_display--;
    delay(refresh_rate);
  ...
  if (temp_select_cal >= temp_max){

    temp_select_cal = temp_max;
    digital_pot = 0;

  } 
  else if (temp_select_cal <= temp_min){
...
void pid_mode(void){
  ...
  if((count_time_delta >= ((time_pressed * 10) - deb_delay)) || (digital_cancel == HIGH) || (pid_error > 10)){
  ...
  if(temp_select_cal < temp_max){
    ...
    //Map = (value - fromLow) * (toHigh - toLow) / (fromHigh - fromLow) + toLow
  ...
  }else if(temp_select_cal == temp_max){
...
// #6 - Mean calculation to diplay the temperature value
void set_temp_display(void){

  if(temp_count < mean_times){
    ...
    temp_cal = temp_sum / mean_times;
    temp_display = temp_cal;
    temp_sum = 0;
    temp_count = 0;
